...
test("ADMIN sends an exact Shaker close-shop follow-up",async t=>{const{request,env,sqlite}=fixture(t);sqlite.exec("INSERT INTO followup_groups(shop_group,telegram_chat_id,telegram_tag) VALUES('SHAKER','-2002','@Deadaluss')");const original=globalThis.fetch;t.after(()=>globalThis.fetch=original);let sent;globalThis.fetch=async(_url,options)=>{sent=JSON.parse(options.body);return Response.json({ok:true,result:{message_id:802}})};const response=await request("/api/followup-requests",{method:"POST",body:JSON.stringify({request_type:"CLOSE_SHOP",shop_group:"SHAKER",shop_name:"shaker044",wallet_number:"17584586544",wallet_type:"Nagad",close_request_type:"Request to close this wallet and withdraw all remaining balance",close_reason:"This wallet will be replaced with a new agent number"})},4);assert.equal(response.status,200);assert.equal(sent.chat_id,"-2002");assert.equal(sent.text,"Shop Name: SHAKER044\nWallet Number: 17584586544\nWallet Type: Nagad\nRequest Type: Request to close this wallet and withdraw all remaining balance\nReason: This wallet will be replaced with a new agent number\nPls check team @Deadaluss thanks");const saved=sqlite.prepare("SELECT user_id,request_type,status FROM followup_requests").get();assert.deepEqual([saved.user_id,saved.request_type,saved.status],[4,"CLOSE_SHOP","SENT"]);});
...
test("follow-up portal uses proper controls and prevents in-flight double submit",()=>{const html=readFileSync(new URL("../docs/index.html",import.meta.url),"utf8"),app=readFileSync(new URL("../docs/app.js",import.meta.url),"utf8");assert.match(html,/id="new-followup"/);assert.match(html,/type="time"/);assert.match(html,/Follow Up History/);assert.doesNotMatch(app,/prompt\(/);assert.match(app,/submit\.disabled=true/);assert.match(app,/\/api\/followup-groups/);assert.match(app,/\/api\/followup-requests/);assert.doesNotMatch(html,/telegram_chat_id|destination_chat_id|telegram_tag/);});
