...
function fixture(t) {
  ...
  sqlite.exec(`
    ...
    CREATE TABLE followup_groups (id INTEGER PRIMARY KEY AUTOINCREMENT, shop_group TEXT NOT NULL UNIQUE, telegram_chat_id TEXT NOT NULL, telegram_tag TEXT NOT NULL, is_active INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);
    CREATE TABLE followup_requests (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, request_type TEXT NOT NULL CHECK (request_type IN ('OFF_WALLET','CLOSE_SHOP')), shop_group TEXT NOT NULL, shop_name TEXT NOT NULL, wallet_number TEXT NOT NULL, wallet_type TEXT NOT NULL, off_from TEXT, current_balance TEXT, b2b_due TEXT, close_request_type TEXT, close_reason TEXT, destination_chat_id TEXT NOT NULL, telegram_tag TEXT NOT NULL, telegram_message_id INTEGER, status TEXT NOT NULL DEFAULT 'PENDING' CHECK (status IN ('PENDING','SENT','FAILED')), error_message TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, sent_at TEXT, FOREIGN KEY (user_id) REFERENCES users(id));
...
test("assignments-only sync leaves follow-up groups untouched",async t=>{const{request,env,sqlite}=fixture(t);env.GSHEET_SYNC_URL="https://sheet.example/exec";env.GSHEET_SYNC_SECRET="test";sqlite.exec("INSERT INTO followup_groups(shop_group,telegram_chat_id,telegram_tag) VALUES('EARTH','-1','@earth_team')");const original=globalThis.fetch;t.after(()=>globalThis.fetch=original);let payload={ok:true,assignments:[{shop_code:"EARTH003",agent_username:"test_agent",active:true}]};globalThis.fetch=async()=>Response.json(payload);const result=await(await request("/api/admin/sync-shop-assignments",{method:"POST"})).json();assert.deepEqual(result.groups,{inserted:0,updated:0,deactivated:0,unchanged:0});assert.equal(sqlite.prepare("SELECT telegram_tag,is_active FROM followup_groups").get().telegram_tag,"@earth_team");payload={...payload,groups:{}};assert.equal((await request("/api/admin/sync-shop-assignments",{method:"POST"})).status,502);assert.equal(sqlite.prepare("SELECT telegram_tag,is_active FROM followup_groups").get().telegram_tag,"@earth_team");});

test("follow-up group sync allows shared chats and updates lifecycle atomically",async t=>{const{request,env,sqlite}=fixture(t);env.GSHEET_SYNC_URL="https://sheet.example/exec";env.GSHEET_SYNC_SECRET="test";sqlite.exec("INSERT INTO followup_groups(shop_group,telegram_chat_id,telegram_tag,is_active) VALUES('EARTH','-old','@old_team',0),('LEGACY','-9','@legacy_team',1)");const original=globalThis.fetch;t.after(()=>globalThis.fetch=original);let payload={ok:true,assignments:[{shop_code:"EARTH003",agent_username:"test_agent",active:true}],groups:[{shop_group:"Earth",chat_id:"-1003878565041",telegram_tag:"@acckayy"},{shop_group:"Shaker",chat_id:"-1003878565041",telegram_tag:"@Deadaluss"}]};globalThis.fetch=async()=>Response.json(payload);let result=await(await request("/api/admin/sync-shop-assignments",{method:"POST"})).json();assert.deepEqual(result.groups,{inserted:1,updated:1,deactivated:1,unchanged:0});assert.deepEqual(sqlite.prepare("SELECT shop_group,telegram_chat_id,telegram_tag,is_active FROM followup_groups ORDER BY shop_group").all().map(x=>[x.shop_group,x.telegram_chat_id,x.telegram_tag,x.is_active]),[["EARTH","-1003878565041","@acckayy",1],["LEGACY","-9","@legacy_team",0],["SHAKER","-1003878565041","@Deadaluss",1]]);result=await(await request("/api/admin/sync-shop-assignments",{method:"POST"})).json();assert.deepEqual(result.groups,{inserted:0,updated:0,deactivated:0,unchanged:3});payload={...payload,groups:[{shop_group:"Earth",chat_id:"-2",telegram_tag:"@earth_new"}]};result=await(await request("/api/admin/sync-shop-assignments",{method:"POST"})).json();assert.deepEqual(result.groups,{inserted:0,updated:1,deactivated:1,unchanged:1});assert.equal(sqlite.prepare("SELECT telegram_chat_id FROM followup_groups WHERE shop_group='EARTH'").get().telegram_chat_id,"-2");payload={...payload,groups:[]};result=await(await request("/api/admin/sync-shop-assignments",{method:"POST"})).json();assert.equal(result.groups.deactivated,1);assert.equal(sqlite.prepare("SELECT COUNT(*) n FROM followup_groups WHERE is_active=1").get().n,0);});

test("invalid follow-up groups reject all assignment and group mutations",async t=>{const{request,env,sqlite}=fixture(t);env.GSHEET_SYNC_URL="https://sheet.example/exec";env.GSHEET_SYNC_SECRET="test";sqlite.exec("INSERT INTO followup_groups(shop_group,telegram_chat_id,telegram_tag) VALUES('EARTH','-1','@earth_team')");const original=globalThis.fetch;t.after(()=>globalThis.fetch=original);const invalid=[[{shop_group:"Earth",chat_id:"-1",telegram_tag:"@earth_team"},{shop_group:"earth",chat_id:"-2",telegram_tag:"@other_team"}],[{shop_group:"",chat_id:"-1",telegram_tag:"@earth_team"}],[{shop_group:"Earth",chat_id:"",telegram_tag:"@earth_team"}],[{shop_group:"Earth",chat_id:-1003878565041,telegram_tag:"@earth_team"}],[{shop_group:"Earth",chat_id:"-1",telegram_tag:""}],[{shop_group:"Earth",chat_id:"-1",telegram_tag:"not-a-tag"}]];for(const groups of invalid){const beforeAssignments=JSON.stringify(sqlite.prepare("SELECT * FROM shop_assignments ORDER BY id").all()),beforeGroups=JSON.stringify(sqlite.prepare("SELECT * FROM followup_groups ORDER BY id").all());globalThis.fetch=async()=>Response.json({ok:true,assignments:[{shop_code:"EARTH020",agent_username:"second",active:true}],groups});const response=await request("/api/admin/sync-shop-assignments",{method:"POST"});assert.equal(response.status,422);assert.equal(JSON.stringify(sqlite.prepare("SELECT * FROM shop_assignments ORDER BY id").all()),beforeAssignments);assert.equal(JSON.stringify(sqlite.prepare("SELECT * FROM followup_groups ORDER BY id").all()),beforeGroups);}}
);

test("follow-up groups are available to ADMIN and AGENT without routing data",async t=>{const{request,sqlite}=fixture(t);sqlite.exec("INSERT INTO followup_groups(shop_group,telegram_chat_id,telegram_tag,is_active) VALUES('EARTH','-1001','@earth_team',1),('SHAKER','-1002','@shaker_team',1),('OLD','-1003','@older_team',0)");for(const user of[1,4]){const response=await request("/api/followup-groups",{},user);assert.equal(response.status,200);const body=await response.json();assert.deepEqual(body.groups,["EARTH","SHAKER"]);assert.equal(JSON.stringify(body).includes("chat"),false);assert.equal(JSON.stringify(body).includes("telegram_tag"),false)}assert.equal((await request("/api/followup-groups",{},null)).status,401);});

test("AGENT sends an exact Earth off-wallet follow-up and preserves text fields",async t=>{const{request,env,sqlite}=fixture(t);sqlite.exec("INSERT INTO followup_groups(shop_group,telegram_chat_id,telegram_tag) VALUES('EARTH','-1001','@acckayy')");const original=globalThis.fetch;t.after(()=>globalThis.fetch=original);let sent;globalThis.fetch=async(_url,options)=>{assert.equal(sqlite.prepare("SELECT status FROM followup_requests").get().status,"PENDING");sent=JSON.parse(options.body);return Response.json({ok:true,result:{message_id:801}})};const response=await request("/api/followup-requests",{method:"POST",body:JSON.stringify({request_type:"OFF_WALLET",shop_group:"earth",shop_name:"earth001",wallet_number:"001458445858",wallet_type:"bk",off_from:"02:33",current_balance:"005800.00",b2b_due:"0"})},1);assert.equal(response.status,200);assert.equal(sent.chat_id,"-1001");assert.equal(sent.text,"Shop Name: EARTH001\nWallet Number: 001458445858\nWallet Type: Bkash\nCurrent Balance: 5800\nShop OFF time from: 02:33\nB2B Due: 0\nPls check team @acckayy thanks");const saved=sqlite.prepare("SELECT * FROM followup_requests").get();assert.equal(saved.user_id,1);assert.equal(saved.wallet_number,"001458445858");assert.equal(saved.status,"SENT");assert.equal(saved.telegram_message_id,801);assert.ok(saved.sent_at);assert.equal(saved.error_message,null);});

test("ADMIN sends custom trimmed Shaker close-shop text unchanged",async t=>{const{request,env,sqlite}=fixture(t);sqlite.exec("INSERT INTO followup_groups(shop_group,telegram_chat_id,telegram_tag) VALUES('SHAKER','-2002','@Deadaluss')");const original=globalThis.fetch;t.after(()=>globalThis.fetch=original);let sent;globalThis.fetch=async(_url,options)=>{sent=JSON.parse(options.body);return Response.json({ok:true,result:{message_id:802}})};const response=await request("/api/followup-requests",{method:"POST",body:JSON.stringify({request_type:"CLOSE_SHOP",shop_group:"SHAKER",shop_name:"shaker044",wallet_number:"17584586544",wallet_type:"Nagad",close_request_type:"  Close this wallet after settlement  ",close_reason:"  Agent number is being replaced  "})},4);assert.equal(response.status,200);assert.equal(sent.chat_id,"-2002");assert.equal(sent.text,"Shop Name: SHAKER044\nWallet Number: 17584586544\nWallet Type: Nagad\nRequest Type: Close this wallet after settlement\nReason: Agent number is being replaced\nPls check team @Deadaluss thanks");const saved=sqlite.prepare("SELECT user_id,request_type,close_request_type,close_reason,status FROM followup_requests").get();assert.deepEqual([saved.user_id,saved.request_type,saved.close_request_type,saved.close_reason,saved.status],[4,"CLOSE_SHOP","Close this wallet after settlement","Agent number is being replaced","SENT"]);});

test("authenticated previews validate and format without persistence or Telegram delivery",async t=>{const{request,sqlite}=fixture(t);sqlite.exec("INSERT INTO followup_groups(shop_group,telegram_chat_id,telegram_tag) VALUES('EARTH','-1001','@acckayy'),('SHAKER','-2002','@Deadaluss')");const original=globalThis.fetch;t.after(()=>globalThis.fetch=original);let sends=0;globalThis.fetch=async()=>{sends++;throw Error("preview must not send")};const off={request_type:"OFF_WALLET",shop_group:"EARTH",shop_name:"EARTH001",wallet_number:"17586958455",wallet_type:"Nagad",off_from:"17:44",current_balance:"2500",b2b_due:"0"},close={request_type:"CLOSE_SHOP",shop_group:"SHAKER",shop_name:"SHAKER005",wallet_number:"1256585455",wallet_type:"Nagad",close_request_type:"Close after the final settlement",close_reason:"Replacing the assigned wallet"};let response=await request("/api/followup-requests/preview",{method:"POST",body:JSON.stringify(off)},1),body=await response.json();assert.equal(response.status,200);assert.equal(body.message,"Shop Name: EARTH001\nWallet Number: 17586958455\nWallet Type: Nagad\nCurrent Balance: 2500\nShop OFF time from: 17:44\nB2B Due: 0\nPls check team @acckayy thanks");const oldPreview=body.message;response=await request("/api/followup-requests/preview",{method:"POST",body:JSON.stringify(close)},4);body=await response.json();assert.equal(response.status,200);assert.equal(body.message,"Shop Name: SHAKER005\nWallet Number: 1256585455\nWallet Type: Nagad\nRequest Type: Close after the final settlement\nReason: Replacing the assigned wallet\nPls check team @Deadaluss thanks");sqlite.exec("UPDATE followup_groups SET telegram_tag='@new_team' WHERE shop_group='EARTH'");response=await request("/api/followup-requests",{method:"POST",body:JSON.stringify({...off,preview_message:oldPreview})},1);assert.equal(response.status,409);assert.equal((await response.json()).error,"FOLLOWUP_PREVIEW_STALE");assert.equal(sqlite.prepare("SELECT COUNT(*) n FROM followup_requests").get().n,0);assert.equal(sends,0);assert.equal((await request("/api/followup-requests/preview",{method:"POST",body:JSON.stringify(off)},null)).status,401);});

test("close-shop manual text is required and whitespace-only values are rejected",async t=>{const{request,sqlite}=fixture(t);sqlite.exec("INSERT INTO followup_groups(shop_group,telegram_chat_id,telegram_tag) VALUES('SHAKER','-2','@team')");const base={request_type:"CLOSE_SHOP",shop_group:"SHAKER",shop_name:"SHAKER1",wallet_number:"01",wallet_type:"Rocket",close_request_type:"Close this wallet",close_reason:"Wallet replacement"};for(const body of[{...base,close_request_type:""},{...base,close_request_type:"   "},{...base,close_reason:""},{...base,close_reason:"   "}])assert.equal((await request("/api/followup-requests/preview",{method:"POST",body:JSON.stringify(body)},1)).status,400);assert.equal(sqlite.prepare("SELECT COUNT(*) n FROM followup_requests").get().n,0);});

test("follow-up validation rejects groups, prefixes, required fields, amounts, wallets, and routing overrides",async t=>{const{request,env,sqlite}=fixture(t);sqlite.exec("INSERT INTO followup_groups(shop_group,telegram_chat_id,telegram_tag,is_active) VALUES('EARTH','-1','@earth_team',1),('SHAKER','-2','@shaker_team',1),('OLD','-3','@older_team',0)");env.TELEGRAM_BOT_TOKEN="test";const original=globalThis.fetch;t.after(()=>globalThis.fetch=original);let sends=0;globalThis.fetch=async()=>{sends++;return Response.json({ok:true,result:{message_id:1}})};const base={request_type:"OFF_WALLET",shop_group:"EARTH",shop_name:"EARTH001",wallet_number:"0012",wallet_type:"Bkash",off_from:"02:33",current_balance:"5800",b2b_due:"0"};const invalid=[{...base,shop_group:"MISSING"},{...base,shop_group:"OLD",shop_name:"OLD1"},{...base,shop_group:"SHAKER"},{...base,shop_name:"SHAKER001"},{...base,wallet_number:""},{...base,wallet_type:"Cash"},{...base,off_from:"25:00"},{...base,current_balance:"-1"},{...base,b2b_due:"-1"},{...base,current_balance:""},{...base,close_reason:"wrong section"},{...base,user_id:2},{...base,destination_chat_id:"-9"},{...base,telegram_tag:"@attacker"},{request_type:"CLOSE_SHOP",shop_group:"EARTH",shop_name:"EARTH1",wallet_number:"1",wallet_type:"Rocket",close_request_type:"",close_reason:""}];for(const body of invalid)assert.notEqual((await request("/api/followup-requests",{method:"POST",body:JSON.stringify(body)},1)).status,200);assert.equal(sends,0);assert.equal(sqlite.prepare("SELECT COUNT(*) n FROM followup_requests").get().n,0);assert.equal((await request("/api/followup-requests",{method:"POST",body:JSON.stringify(base)},null)).status,401);});

test("Telegram failure preserves a safe FAILED follow-up record",async t=>{const{request,env,sqlite}=fixture(t);sqlite.exec("INSERT INTO followup_groups(shop_group,telegram_chat_id,telegram_tag) VALUES('EARTH','-1','@earth_team')");const original=globalThis.fetch;t.after(()=>globalThis.fetch=original);globalThis.fetch=async()=>Response.json({ok:false,description:"secret upstream detail"},{status:403});const body={request_type:"OFF_WALLET",shop_group:"EARTH",shop_name:"EARTH1",wallet_number:"01",wallet_type:"Rocket",off_from:"00:01",current_balance:"0",b2b_due:"0"};const response=await request("/api/followup-requests",{method:"POST",body:JSON.stringify(body)},1);assert.equal(response.status,502);assert.equal((await response.json()).error,"TELEGRAM_SEND_FAILED");const saved=sqlite.prepare("SELECT status,error_message,telegram_message_id,sent_at FROM followup_requests").get();assert.equal(saved.status,"FAILED");assert.equal(saved.error_message,"Telegram rejected request (HTTP 403)");assert.equal(saved.telegram_message_id,null);assert.equal(saved.sent_at,null);assert.equal(JSON.stringify(saved).includes("secret upstream detail"),false);});
...
test("localization and application declarations can bootstrap together",()=>{const source=readFileSync(new URL("../docs/i18n.js",import.meta.url),"utf8"),html=readFileSync(new URL("../docs/index.html",import.meta.url),"utf8"),values=new Map(),document={body:{},documentElement:{},title:"",querySelectorAll:()=>[],createTreeWalker:()=>({nextNode:()=>null})},window={},context=createContext({localStorage:{getItem:key=>values.get(key)||null,setItem:(key,value)=>values.set(key,value)},document,window,NodeFilter:{SHOW_TEXT:4},Set,WeakMap});runInContext(source,context);assert.doesNotThrow(()=>runInContext('const {t,applyLanguage,setLanguage}=window.opsI18n; window.loginBootstrapReady=typeof t==="function"&&typeof applyLanguage==="function"&&typeof setLanguage==="function";',context));assert.equal(window.loginBootstrapReady,true);assert.ok(html.indexOf('./i18n.js')<html.indexOf('./app.js'));});
