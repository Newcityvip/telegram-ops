...
export async function respondToCase(request, env, user, caseId) {
  ...
  try {
    ...
    if (!telegram.ok || result?.ok !== true || !Number.isSafeInteger(result?.result?.message_id)) throw Error(result?.description || "Telegram rejected response");
    telegramMessageId = result.result.message_id;
  } catch (error) {
    await env.DB.prepare("UPDATE responses SET status = 'FAILED', error_message = ? WHERE id = ? AND status = 'PENDING'").bind("Telegram send failed", responseId).run();
