...
export async function resolveResponseRoute(db, shopCode) {
  const shop = String(shopCode || "").trim().toUpperCase();
  const rows = (await db.prepare("SELECT id,shop_group,telegram_chat_id FROM followup_groups WHERE is_active=1").all()).results;
  const matches = rows.filter((row) => { const group = String(row.shop_group || "").trim().toUpperCase(); return group && shop.startsWith(group); });
  if (matches.length === 0) return { error: "RESPONSE_ROUTE_UNAVAILABLE", status: 409 };
  if (matches.length !== 1) return { error: "RESPONSE_ROUTE_AMBIGUOUS", status: 409 };
  return { route: matches[0] };
}

export async function respondToCase(request, env, user, caseId) {
  if (!["AGENT", "ADMIN"].includes(user.role)) return { error: "FORBIDDEN", status: 403 };
  ...
  try {
    item = await env.DB.prepare(`
      SELECT c.id, c.shop_code, c.raw_message, c.matched_rule_id, c.assigned_user_id,
        r.response_type, r.response_config
      FROM cases c
  ...
  if (!item || (user.role === "AGENT" && item.assigned_user_id !== user.id)) return { error: "CASE_NOT_FOUND", status: 404 };
  ...
  let destination;
  try {
    const resolved = await resolveResponseRoute(env.DB, item.shop_code);
  ...
  try {
    const caseUpdate = user.role === "AGENT"
      ...
      : env.DB.prepare("UPDATE cases SET status = 'ANSWERED', answered_at = ?, updated_at = ? WHERE id = ?").bind(sentAt, sentAt, caseId);
    await env.DB.batch([
      ...
      env.DB.prepare("INSERT INTO audit_logs (user_id, case_id, action, entity_type, entity_id, new_value, metadata) VALUES (?, ?, 'CASE_RESPONSE_SENT', 'RESPONSE', ?, ?, ?)")
        .bind(user.id, caseId, String(responseId), selected, JSON.stringify({ matched_rule_id: item.matched_rule_id, response_group_id: destination.id, response_group_name: destination.shop_group, telegram_message_id: telegramMessageId }))
    ]);
  } catch {
